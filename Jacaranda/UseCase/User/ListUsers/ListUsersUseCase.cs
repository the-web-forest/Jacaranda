...
namespace Jacaranda.UseCase.ListUsers
{
    public class ListUsersUseCase : IUseCase<ListUsersUseCaseInput, ListUsersUseCaseOutput>
    {
        ...
        private async Task<List<Models.User>> GetUsers(int Page)
        {
            return await _userRepository.ListUsersPerPage(Page, ITENS_PER_REQUEST);
        }

        private ListUsersUseCaseOutput BuildResponse(int CurrentPage, long TotalPages, List<Models.User> Users)
        {
